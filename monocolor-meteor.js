{
    ...
    const meteor = (dt) => {
        ...
        if (wavefrontPosition - wavefrontLength >= length) wavefrontPosition -= wavefrontSeparation

        let nthWavefrontPosition = wavefrontPosition
        while(nthWavefrontPosition >= 0){
            for(let i = 0;i<wavefrontLength;i++){
                if(Math.round(nthWavefrontPosition - i) >= 0) buf[Math.round(nthWavefrontPosition - i)] = color
            }
